...
import Room, { IImages, IRoom } from '../models/room';
...
export const allRooms = catchAsyncErrors(async (req: NextRequest) => {
  ...
  let rooms: IRoom[] = await apiFilters.query.clone();
...
// Delete room image => /api/admin/rooms/:id/delete_images
  ...
  async (req: NextRequest, { params }: { params: { id: string } }) => {
    ...
    if (isDeleted) {
      room.images = room?.images?.filter(
        (img: IImages) => img?.public_id !== body.imgId
      );
    }
...
// Create/update room reviews => /api/reviews
...
// Get room reviews => /api/admin/rooms/reviews
...
export const deleteRoomReview = catchAsyncErrors(async (req: NextRequest) => {
  ...
  // TODO: create function that calculate the rating
  // because now we have two the same part of code

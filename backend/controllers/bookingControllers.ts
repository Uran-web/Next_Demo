...
const getLastSecMonthsSales = async () => {
...
const getTopPerformingRooms = async (startDate: Date, endDate: Date) => {
  const topRooms = await Booking.aggregate([
    // Stage 1: Filter by date. Greater than start date and lower than endDate
    {
      $match: {
    ...
    // Stage 2: Group by room id and summarize the bookings
    {
      $group: {
        _id: '$room',
        bookingsCount: { $sum: 1 },
    ...
    // Stage 7: Shape output document (include or exclude the fields)
    {
      $project: {
        _id: 0,
...
export const getSalesStats = catchAsyncErrors(
  async (req: NextRequest, { params }: { params: { id: string } }) => {
    ...
    const sixMonthSalesData = await getLastSecMonthsSales();
...
export const deleteBooking = catchAsyncErrors(
  async (req: NextRequest, { params }: { params: { id: string } }) => {
    ...
    if (!booking) {
      throw new ErrorHandler('Booking not founded with this ID ', 404);
    }
